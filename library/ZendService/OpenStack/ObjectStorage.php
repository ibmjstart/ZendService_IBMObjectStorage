...
class ObjectStorage extends AbstractOpenStack
{
    ...
    public function __construct(array $options, HttpClient $httpClient = null)
    {
        ...
        if (!$this->encodedAuth($auth)) {
            throw new Exception\RuntimeException(
                "Couldn't retrieve access token. This usually means that your Object Storage account with Softlayer is still being provisioned. Please check back soon."
            );
        }
    ...
    /**
    * Get object URL for adding an object.
    *
    * @param string $bucket
    * @param string $object
    * @return string
    */
    public function getObjectUrl( $bucket, $object ){
      $objectURL = $this->storage_url . '/' . $bucket . '/' . $object;
      return $objectURL;
    }

    /**
    * Find the softlayer storage URL returned by authentication.
    *
    * @return string
    */
    ...
    protected function auth($username, $password, $key = null)
    {
        ...
        $this->storage_url = $headers[self::HEADER_STORAGEURL];

        return true;
    }

    /**
    * Alternate authentication using basic authorization
    * @param string $encoded
    * @return bool
    */
    protected function encodedAuth($encoded){
      $result = $this->api->auth($encoded);
      if (!$this->api->isSuccess()) {
          return false;
      }
    ...
    protected function setOptions(array $options)
    {
        ...
        if (!isset($options['url']) || !isset($options['user']) || !isset($options['key'])) {
            throw new Exception\InvalidArgumentException(
                'Could not retrieve VCAP_SERVICES credentials. Check that Object Storage is still bound to your service.'
            );
        }
